...
var migrationReferences = []Reference{
	{
		...
		Intro: "The migration program's SOP, in execution order: branch, swap the " +
			...
			"Every rule in here was paid for. The dependency swap has one wrong turn " +
			"that breaks the FX graph; the handler conversion has two mistakes that " +
			"fail only at runtime; the validation step has two more that fail only " +
			"when a request arrives. Follow the order, and treat the CRITICAL rules " +
			"as blocking.\n\n" +
			"Run `legacy_audit` before starting and after finishing \u2014 it should go " +
			"from a page of findings to none.",
		Body: `## What the conversion changes, in one screen
