...
// The reference in this file is the migration SOP: the operations team's
// step-by-step procedure for converting a legacy api-* service to the
// n-api template, hand-authored here the same way references_review.go
// carries the code-review findings.
//
// It is a third body of knowledge, distinct from the other two. skill.md
// teaches the *destination* — what a finished n-api service looks like —
// and the legacy rules detect the *departure point*. Neither says how to
// travel: which dependency swap breaks the FX graph if done wrong, why a
// raw-slice handler parameter fails at runtime rather than compile time,
// which HTTP status the migrated validation returns. Those lessons came
// from the conversion program itself, and they are exactly the mistakes an
// agent repeats when it migrates a service with only the destination in
// view.
//
...
var migrationReferences = []Reference{
	{
		...
		Intro: "The migration program's SOP, in execution order: branch, swap the " +
			...
			// nowhere else in the document -- so a Planner that read this
			// paragraph went looking for a set of rules the corpus did not
			// contain. Worse, this intro is a near-perfect lexical match for
			// every query the paragraph inspires (handler, conversion,
...
func init() { References = append(References, migrationReferences...) }
